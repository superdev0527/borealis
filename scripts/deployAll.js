...
async function main() {
    ...
    const daiBond = await DAIBond.deploy(ohm.address, dai.address, treasury.address, "0xCdCFc0f66c522Fd086A1b725ea3c0Eeb9F9e8814", zeroAddress);

    // Deploy Frax bond
    //@dev changed function call to Treasury of 'valueOf' to 'valueOfToken' in BondDepository due to change in Treausry contract
    const FraxBond = await ethers.getContractFactory('MockOlympusBondDepository');
    const fraxBond = await FraxBond.deploy(ohm.address, frax.address, treasury.address, "0xCdCFc0f66c522Fd086A1b725ea3c0Eeb9F9e8814", zeroAddress);
